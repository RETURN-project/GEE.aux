...
# name of data files
ifile <- 'samples_forest_fire_nat_n1000_' # base name of the input file
...
for (i in seq(startPix,endPix,by =intPix)){
  ...
  write_csv(dat_tidy, file.path(ofolder, paste0('tidy_',ifile, i, '_', endpnt, '.csv')))
  save(dat_tidy, file = file.path(ofolder, paste0('tidy_', ifile, i, '_', endpnt, '.Rdata')))
  ...
  save(col_names, file = file.path(ofolder, paste0('col_names_', ifile, i, '_', endpnt, '.Rdata')))
...
for (i in seq(startPix,endPix,by =intPix)){
  if(i == endPix){endpnt = 'None'}else{endpnt = i+intPix}
  # load data
  load(file.path(ofolder, paste0('tidy_', ifile, i, '_', endpnt, '.Rdata')))
  load(file.path(ofolder, paste0('col_names_', ifile, i, '_', endpnt, '.Rdata')))
  ...
  # save results
  save(fireDts, file = file.path(ofolder, paste0('fire_', ifile, i, '_', endpnt, '.Rdata')))# save the result
...
for (i in seq(startPix,endPix,by =intPix)){
  ...
  save(df_out, file = file.path(ofolder, paste0('reg_', ifile, i, '_', endpnt, '.Rdata')))# save the result
...
for (i in seq(startPix,endPix,by =intPix)){
  if(i == endPix){endpnt = 'None'}else{endpnt = i+intPix}
  load(file.path(ofolder, paste0('reg_', ifile, i, '_', endpnt, '.Rdata')))
  ...
  # save result
  save(seg, file = file.path(ofolder, paste0('seg_', ifile, i, '_', endpnt, '.Rdata')))
  rm(nts, df_out, seg)
}
```
...
for (i in seq(startPix,endPix,by =intPix)){
  if(i == endPix){endpnt = 'None'}else{endpnt = i+intPix}
  load(file.path(ofolder,paste0('rec_', ifile, i, '_', endpnt, '.Rdata')))
  load(file.path(ofolder,paste0('seg_', ifile, i, '_', endpnt, '.Rdata')))
  load(file.path(ofolder,paste0('reg_', ifile, i, '_', endpnt, '.Rdata')))
  load(file.path(ofolder,paste0('fire_', ifile, i, '_', endpnt, '.Rdata')))
  ...
  df_lst <-  lapply(1:nts,function(i){
    ...
    rec <- sapply(1:length(fdts), function(ii){
      if(!is.na(tdist[ii])){
        ...
        list('RRI' = NA, 'R80P' = NA, 'YrYr' = NA , 'impact' = NA, 'Vpre' = NA)
      }})
  })
  ...
  # save result
  save(df_rec, file = file.path(ofolder,  paste0('rec_', ifile, i, '_', endpnt, '.Rdata')))
  rm(df_out, nts, dts, df_lst, fdts, fireDts, tdist, tsi, seg, tbp, rec, df_fr, df_rec)
}
```
