...
# name of data files
ifile <- 'samples_forest_fire_nat_n1000_26000_28000.csv'
...
# save results
save(fireDts, file = file.path(ifolder, paste0('fire_', tools::file_path_sans_ext(ifile), '.Rdata')))# save the result
...
df_lst <-  lapply(1:nts,function(i){
  ...
  rec <- sapply(1:length(fdts), function(ii){
    if(!is.na(tdist[ii])){
      ...
      list('RRI' = NA, 'R80P' = NA, 'YrYr' = NA , 'impact' = NA)
    }})
})
...
```{r shp}
library(raster)
df_rec_val <- df_rec[complete.cases(df_rec),]
...
raster::shapefile(df_rec_val, file.path(ifolder,"samples_forest_fire_nat_n1000_VAL.shp"))
